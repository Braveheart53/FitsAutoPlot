"""
CSV/TSV AutoPlot V5 - Automatic data visualization with Veusz Embedded API.
...
VERSION 5 (2026-02-03) - ENHANCED MULTI-FILE SUPPORT:
...
- Independent X-axis data per file (each file uses its own X data)
...
- Page notes with file metadata from header lines
- Logging checkbox and log file creation
- Overlay plot of all imported data
...
class VeuszPlotter:
    ...
    def set_logging(self, enable: bool, log_path: str = None):
        ...
        if enable and log_path:
            try:
                ...
                self.log_file.write(f"CSV/TSV AutoPlot V5 Log - {timestamp}\n")
    ...
    def create_overlay_and_individual_plots(
        ...
        """
        Create both overlay plot (all files) and individual plots (one per file).
        ...
        try:
            # Create page
            page = self.doc.Root.Add('page', name='AllPlots')
            
            # Add page notes with metadata from all files
            ...
            total_plots = 1 + num_files  # 1 overlay + individual plots
            
            # Arrange in grid: 2 columns
            num_cols = 2
            num_rows = (total_plots + num_cols - 1) // num_cols
            
            grid = page.Add('grid', columns=num_cols, rows=num_rows)
            
            # Color palette for plots
            colors = ['blue', 'red', 'green', 'orange', 'purple', 'brown', 
                     'cyan', 'magenta', 'yellow', 'black']
            
            # ========== CREATE OVERLAY PLOT (all files) ==========
            overlay_graph = grid.Add('graph', name='OverlayPlot')
            ...
            for filename, df in data_dict.items():
                ...
                for y_col in y_columns:
                    ...
                    with self._wrap_widget(xy_plot) as plot:
                        plot.PlotLine.color.val = colors[plot_idx % len(colors)]
            ...
            for filename, df in data_dict.items():
                ...
                individual_graph = grid.Add('graph', name=f'Plot_{base_filename}')
                ...
                for y_col in y_columns:
                    ...
                    xy_plot = individual_graph.Add('xy',
                ...
                with self._wrap_widget(individual_graph) as g:
                    g.Add('label', name='plotTitle')
                    g.plotTitle.Text.size.val = '12pt'
...
class CSVAutoPlotMainWindow(QMainWindow):
    """Main application window for CSV/TSV AutoPlot V5."""
    
    def __init__(self):
        """Initialize the main window."""
        super().__init__()
        self.setWindowTitle("CSV/TSV AutoPlot V5 - Data Visualization Tool")
        ...
        # Setup UI
        self.setup_ui()
        self.log_message("CSV/TSV AutoPlot V5 initialized successfully")
    ...
    def setup_ui(self):
        ...
        self.enable_logging_checkbox.setToolTip("Log missing data warnings to file")
        ...
        selection_form.addRow("X-Axis Column (common to all files):", self.x_column_combo)
        
        # X-axis validation warning label
        self.x_axis_warning_label = QLabel("")
        self.x_axis_warning_label.setStyleSheet("color: red; font-weight: bold;")
    ...
    def load_files(self):
        ...
        for file_path in self.selected_files:
            if file_path not in [os.path.join(d, f) for d, f in
                ...
                df, msg, metadata_lines = load_csv_file(file_path, delimiter, self.csv_config)
                if df is not None:
    ...
    def update_column_selectors(self):
        ...
        if common_cols_sorted:
            ...
            self.log_message(f"X-axis options: {len(common_cols_sorted)} common columns")
        else:
            self.log_message("WARNING: No common columns found across all files!")
        
        # Populate Y-axis list with all columns
        if all_cols_sorted:
            self.y_columns_list.addItems(all_cols_sorted)
            self.log_message(f"Y-axis options: {len(all_cols_sorted)} total columns")
    ...
    def validate_x_axis_selection(self):
        ...
        if not x_column or not self.loaded_data:
            ...
            self.generate_btn.setStyleSheet("background-color: gray; color: white; font-weight: bold; padding: 8px;")
        ...
        if missing_files:
            ...
            self.generate_btn.setEnabled(False)
            self.generate_btn.setStyleSheet("background-color: #DC143C; color: white; font-weight: bold; padding: 8px;")
            ...
            self.generate_btn.setStyleSheet("background-color: #4CAF50; color: white; font-weight: bold; padding: 8px;")
            self.x_axis_warning_label.setText("")
            self.log_message(f"X-axis validation PASSED: '{x_column}' exists in all {len(self.loaded_data)} files")
    ...
    def generate_plots(self):
        ...
        try:
            ...
            if self.enable_logging_checkbox.isChecked():
                ...
                log_path = f"CSV_AutoPlot_V5_Log_{timestamp}.txt"
            ...
            if success:
                ...
                QMessageBox.information(self, "Success", 
                    "Overlay and individual plots created successfully!\n"
                    "Each file uses its own independent X-axis data.\n"
                    "Use 'Save to Veusz' button to save the project.")
            else:
                QMessageBox.critical(self, "Error", "Failed to create plots in Veusz")
        ...
        if file_path:
            try:
                ...
                if success:
                    ...
                    QMessageBox.critical(self, "Error", "Failed to save Veusz project")
            except Exception as e:
