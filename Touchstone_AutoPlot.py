#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Enhanced Touchstone AutoPlot V3 - Complete Production Version with Windows Long Path Support.
...
- ✅ V3: Windows long path support (>260 chars) using pathlib and \\\\?\\ prefix
- ✅ V3: Path length validation before all file operations
- ✅ V3: Short temp directory paths to reduce accumulation
- ✅ V3: Enhanced error handling for path-related issues
- ✅ V3: Cross-platform path handling with pathlib throughout
Author: William W. Wallace
Last updated: 2026-02-03 (V3 - COMPLETE LONG PATH SUPPORT INTEGRATED)
"""
...
# ✅ FIXED V3: Long path issue resolved with extended-length path support
# ============================================================================
...
# ============================================================================
# WINDOWS LONG PATH SUPPORT UTILITIES (V3)
# ============================================================================
...
def validate_path_length(path: Union[str, Path], operation: str = "operation") -> tuple:
    """Validate that path length is within limits.
    ...
    if is_windows() and not path_str.startswith(EXTENDED_PATH_PREFIX):
        if path_len >= MAX_PATH_WINDOWS:
            return False, f"Path too long for {operation} ({path_len} chars, limit {MAX_PATH_WINDOWS}).\nPath: {path_str[:100]}..."
        elif path_len >= PATH_WARNING_THRESHOLD:
            return True, f"Warning: Path approaching limit ({path_len}/{MAX_PATH_WINDOWS} chars) for {operation}."
...
class ProcessingConfig:
    ...
    use_extended_paths: bool = is_windows()  # V3: Enable long path support by default on Windows
    validate_path_lengths: bool = True  # V3: Enable path length validation
@dataclass
...
class SmithChartPlottermpld3:
    ...
    def create_pdf_with_bookmarks(pdf_files: List[Tuple[str, str]], output_path: str) -> bool:
        """Merge PDFs with bookmarks for navigation (V3: with long path support).
        Parameters
        ----------
...
class TouchstonePlotter:
    ...
    def save(self, filename: str):
        """Save Veusz project (V3: with long path support)."""
        # V3: Normalize path
        filename = str(normalize_path(filename))

        # V3: Validate path length
        is_valid, msg = validate_path_length(filename, "Veusz project save")
        if not is_valid:
            raise OSError(msg)
        elif msg:
            print(msg)
...
def process_single_touchstone_file(file_info: Tuple) -> Tuple[str, Optional[dict]]:
    ...
    try:
        ...
        data = {
            ...
            'file_path': filepath_normalized,  # V3: Use normalized path
...
# ============================================================================
def main():
    """Main application entry point (V3: with long path support)."""
    print("="*80)
    print("Touchstone AutoPlot V3 - Windows Long Path Support Enabled")
    print("="*80)
