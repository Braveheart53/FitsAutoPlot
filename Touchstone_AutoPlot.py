#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Enhanced Touchstone AutoPlot V4 - Complete Production with Windows Long Path Support.
...
- FIXED: Unwrapped phase datasets now end with _unwrapped suffix
- FIXED: Frequency domain FFT calculation no longer truncated
- FIXED: Trace visibility improved with better transparency and markers
Author: William W. Wallace
Last updated: 2026-02-03 (V4: Windows Long Path Support + Python 3.12 Compatible)
"""
...
# ============================================================================
# V4: WINDOWS LONG PATH SUPPORT - CONSTANTS AND UTILITIES
# ============================================================================
...
def get_extended_path(path: Union[str, Path]) -> Path:
    """
    Convert path to Windows extended-length format (\\?\) if needed.

    On Windows, paths starting with \\?\ can be up to 32,767 characters.
    """
    ...
    if path_obj.is_absolute():
        return Path(f"{EXTENDED_PATH_PREFIX}{path_str}")
...
def normalize_path(path: Union[str, Path]) -> Path:
    """
    Normalize path for safe file operations with long path support.
    """
    if is_windows():
...
def validate_path_length(path: Union[str, Path], operation: str = "operation") -> tuple:
    """
    Validate path length and provide warnings/errors.
    """
    path_str = str(path)
    ...
    if path_len >= MAX_PATH_WINDOWS:
        return False, (
            f"ERROR: Path too long for {operation} ({path_len}/{MAX_PATH_WINDOWS} chars).\n"
        ...
        return True, (
            f"Warning: Path approaching limit ({path_len}/{MAX_PATH_WINDOWS} chars) for {operation}.\n"
...
def create_short_temp_dir() -> Path:
    """
    Create a temporary directory with SHORT path on Windows.
    """
    if is_windows():
        try:
            ...
            temp_dir = base_temp / f"TS_{random_suffix}"
    ...
    temp_path = Path(tempfile.mkdtemp(prefix="Touchstone_"))
...
class ProcessingConfig:
    ...
    # V4: Windows Long Path Support
    use_extended_paths: bool = is_windows()
    validate_path_lengths: bool = True
@dataclass
        ...
        try:
            ...
            if export_format.lower() == "html":
                if not MPLD3_AVAILABLE:
                    ...
                    fig.savefig(output_path_normalized.replace('.html', '.png'),
                ...
                fig.savefig(output_path_normalized, format='png', dpi=150, bbox_inches='tight')
            elif export_format.lower() == "svg":
                fig.savefig(output_path_normalized, format='svg', bbox_inches='tight')
            elif export_format.lower() == "pdf":
                fig.savefig(output_path_normalized, format='pdf', bbox_inches='tight')
            else:
...
class TouchstonePlotter:
    ...
    def _create_frequency_page(self, dataset_name, data):
        ...
        ✅ FIXED: Phase unwrapping logic corrected - wrapped phase only calculated in else block.
        ✅ FIXED: Dataset names now end with _unwrapped when phase is unwrapped.
        """
        network = data['network']
        ...
        for i in range(n_ports):
            for j in range(n_ports):
                ...
                if self.unwrap_phase:
                    ...
                    # ✅ FIXED: Calculate wrapped phase only if NOT unwrapping
                    phase_deg = np.angle(s_param, deg=True)
    ...
    def _create_time_gated_page(self, dataset_name, param_name, data, td_result,
        ...
        ✅ FIXED: Improved trace visibility with better transparency and markers.
        Parameters
        ----------
        plot_time_domain : bool
        ...
        if 'header_info' in data:
            page_tg.notes.val = '\n'.join(data['header_info'])
        # TIME DOMAIN PLOT - ✅ FIXED: Include both unfiltered and gated with improved visibility
        if plot_time_domain:
            ...
            # Plot unfiltered response (dotted line with markers for visibility)
            ...
            with self._wrap_widget(xy_unfilt) as plot:
                ...
                plot.PlotLine.color.val = 'blue'
                plot.marker.val = 'circle'
                plot.markerSize.val = '1.5pt'
                plot.MarkerFill.color.val = 'blue'
                plot.MarkerLine.color.val = 'blue'
            # Plot gated response (solid line, 40% transparent for better visibility - 60% opacity)
            xy_gated = graph_tg.Add('xy', name=f"{param_name}_timegated_gated")
            self._configure_xy_plot(xy_gated, time_name, td_gated_name, get_sparam_color(param_name), transparency=40)
        ...
        if plot_frequency_domain:
            ...
            if len(original_frequencies) > 1:
                freq_spacing_hz = np.mean(np.diff(original_frequencies))
            else:
                freq_spacing_hz = 1.0  # Default fallback
            num_fft = len(td_unfilt_data)
            # Create proper FFT frequency axis in Hz, then convert to GHz
            # Time-domain record length determines frequency resolution
            dt = 1.0 / (len(original_frequencies) * freq_spacing_hz) if freq_spacing_hz > 0 else 1.0
            fft_freqs_hz = np.fft.fftfreq(num_fft, dt)
            ...
            self._configure_standard_graph(graph_tg_freq, graph_title_freq, 'Frequency (GHz)', 'Magnitude (dB)', -80,
                                           20)
            # Plot unfiltered FFT (dotted line with markers)
    ...
    def save(self, filename: str):
        """Save Veusz project (V4: with long path support)."""
        # V4: Normalize and validate path
        filename_normalized = str(normalize_path(filename))
        is_valid, msg = validate_path_length(filename_normalized, "Veusz project save")
        ...
        self.doc.Save(filename_normalized, mode='hdf5')
# ============================================================================
...
class TouchstonePlotCanvas(FigureCanvas):
    ...
    def plot_time_domain(self, time, td_data, td_filtered_data=None, title="Time Domain"):
        ...
        if td_filtered_data is not None:
            ax.plot(time, np.abs(td_filtered_data), "-", linewidth=2, label="Gated (40% transparent)", alpha=0.6)
...
def process_single_touchstone_file(file_info: Tuple) -> Tuple[str, Optional[dict]]:
    """Worker function to process a single Touchstone file (V4: with long path support)."""
    filepath, _ = file_info
    try:
        # V4: Normalize path FIRST - creates the variable
        filepath_normalized = str(normalize_path(filepath))

        # V4: Validate path length before processing
        ...
        # V4: Use normalized path for file operations
        network = Network(filepath_normalized)
        filename = os.path.basename(filepath)
        header_info = []
        try:
            with open(filepath_normalized, 'r', encoding='ascii', errors='ignore') as f:
                for line in f:
...
class TouchstoneMainWindow(QMainWindow):
    ...
    def setup_time_domain_tab(self):
        ...
        # ✅ PLOT OUTPUT OPTIONS - FIXED: Checkboxes properly connected to button state
        ...
        # ✅ FIXED: Connect checkbox state changes to button state update
        self.plot_time_domain_checkbox.stateChanged.connect(self._update_timegated_button_state)
        self.plot_frequency_domain_checkbox.stateChanged.connect(self._update_timegated_button_state)
...
def main():
    """Main application entry point."""
    # ============================================================================
    # V4: DISPLAY PATH SUPPORT STATUS
    # ============================================================================
    print("=" * 80)
    print("Touchstone AutoPlot V4 - Windows Long Path Support Enabled")
    print("=" * 80)
    print(f"Platform: {platform.system()}")
    print(f"Python: {sys.version.split()[0]}")
    print(f"Extended path support: {'Enabled' if is_windows() else 'Not needed (non-Windows)'}")
    if is_windows():
        print(f"MAX_PATH limit: {MAX_PATH_WINDOWS} characters")
        print(f"Warning threshold: {PATH_WARNING_THRESHOLD} characters")
    print("=" * 80)
    print()
